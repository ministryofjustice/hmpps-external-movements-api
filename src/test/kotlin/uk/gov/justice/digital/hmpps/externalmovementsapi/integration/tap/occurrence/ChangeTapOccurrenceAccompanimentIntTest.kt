...
class ChangeTapOccurrenceAccompanimentIntTest(
  ...
  fun `200 ok single tap occurrence accompaniment updated successfully`() {
    ...
    verifyEventPublications(
      saved,
      setOf(
        TemporaryAbsenceAccompanimentChanged(occurrence.authorisation.person.identifier, occurrence.id).publication(occurrence.id),
        TemporaryAbsenceAuthorisationAccompanimentChanged(auth.person.identifier, auth.id).publication(auth.id),
      ),
    )
  }
