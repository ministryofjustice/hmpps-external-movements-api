...
class RescheduleTapOccurrenceIntTest(
  ...
  fun `200 ok single tap occurrence rescheduled successfully`() {
    ...
    verifyEventPublications(
      saved,
      setOf(
        TemporaryAbsenceRescheduled(occurrence.authorisation.person.identifier, occurrence.id).publication(occurrence.id),
        TemporaryAbsenceAuthorisationDateRangeChanged(auth.person.identifier, auth.id).publication(auth.id),
      ),
    )
  }
