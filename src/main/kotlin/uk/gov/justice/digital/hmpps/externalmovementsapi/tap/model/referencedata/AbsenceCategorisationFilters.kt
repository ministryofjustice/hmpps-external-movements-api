...
import uk.gov.justice.digital.hmpps.externalmovementsapi.tap.domain.referencedata.DomainLinkedReferenceData
import uk.gov.justice.digital.hmpps.externalmovementsapi.tap.domain.referencedata.absencereason.AbsenceCategorisationLink
  ...
  companion object {
    fun of(
      ...
      return if (head is DomainLinkedReferenceData && head.nextDomain != null && links.size > 1) {
        Node(domainCode, head, links, prefixOverride, positionOverride ?: FilterPosition.positionOf(head))
      } else {
        links.singleOrNull()?.let {
          of(
            ...
            linked,
            head.parentPrefix(),
            positionOverride ?: FilterPosition.positionOf(head),
          )
        } ?: Leaf(domainCode, head, prefixOverride, positionOverride ?: FilterPosition.positionOf(head))
      }
    }
  }
}
