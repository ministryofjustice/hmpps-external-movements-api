...
class SyncScheduledTemporaryAbsence(
  ...
  private fun ScheduledTemporaryAbsenceRequest.asEntity(
    ...
    accompaniedBy = escort?.let { rdProvider(ReferenceDataDomain.Code.ACCOMPANIED_BY, it) as AccompaniedBy },
    transport = transportType?.let { rdProvider(ReferenceDataDomain.Code.TRANSPORT, it) as Transport },
    notes = comment,
