...
fun occurrenceMatchesPrisonCode(prisonCode: String) = Specification<TemporaryAbsenceOccurrence> { tao, _, cb ->
  cb.equal(tao.get<String>(TemporaryAbsenceOccurrence.PRISON_CODE), prisonCode)
}
...
fun occurrenceOverlapsDateRange(start: LocalDate?, end: LocalDate?) = Specification<TemporaryAbsenceOccurrence> { tao, _, cb ->
  cb.and(
    start?.let { cb.greaterThanOrEqualTo(tao.get(TemporaryAbsenceOccurrence.END), it) } ?: cb.conjunction(),
    end?.let { cb.lessThanOrEqualTo(tao.get(TemporaryAbsenceOccurrence.START), it.plusDays(1)) } ?: cb.conjunction(),
  )
}
...
fun occurrenceStatusCodeIn(statusCodes: Set<OccurrenceStatus.Code>) = Specification<TemporaryAbsenceOccurrence> { tao, _, _ ->
  val status = tao.join<TemporaryAbsenceOccurrence, OccurrenceStatus>(TemporaryAbsenceOccurrence.STATUS, JoinType.INNER)
...
fun startAfter(start: LocalDateTime) = Specification<TemporaryAbsenceOccurrence> { tao, _, cb ->
  cb.greaterThanOrEqualTo(tao.get(TemporaryAbsenceOccurrence.START), start)
}

fun startBefore(end: LocalDateTime) = Specification<TemporaryAbsenceOccurrence> { tao, _, cb ->
  cb.lessThan(tao.get(TemporaryAbsenceOccurrence.START), end)
}
...
fun occurrenceIsAccompanied(accompanied: Boolean) = Specification<TemporaryAbsenceOccurrence> { tao, _, cb ->
  val accompaniedBy = tao.join<TemporaryAbsenceOccurrence, AccompaniedBy>(TemporaryAbsenceOccurrence.ACCOMPANIED_BY, JoinType.INNER)
